...
if (!$model->isNewRecord) {
    $formId = 'worksUpdate';
    $action = '/works/update?id=' . $model->id;
    $defaultImage = Files::getDefaultImageIdByProductId($model->id, 'work');
} else {
    $formId = 'worksCreate';
    $action = '/works/create';
}
...
<div class="categoyr-form">
    <?= Html::a('Back to work list', ['/works/index'], ['class' => 'btn btn-primary mb15']) ?>
    <div class="panel sort-disable mb50" id="p2" data-panel-color="false" data-panel-fullscreen="false" data-panel-remove="false" data-panel-title="false">
        <div class="panel-heading">
            <span class="panel-title"><?php echo Yii::t('app', 'Add New work') ?></span>
            ...
            <ul class="nav panel-tabs-border panel-tabs">
                ...
                if (!$model->isNewRecord) {
                    foreach ($languages as $value):
                        ...
                        ?>">
                            <a href="#tab_<?php echo $value['id'] ?>"  data-toggle="tab" onclick="editWorkTr(<?php echo $value['id']; ?>,<?php echo $model->id; ?>,<?php echo $value['is_default']; ?>)" disabled="disabled">
        ...
        <div class="panel-body"  style="display: block;">
            <div class="tab-content pn br-n admin-form">
                <div class="tab-pane" id="tr_work"></div>
                <div class="tab-pane active" id="tab_<?php echo $defoultId; ?>">
                    ...
                    <div class="tab-content row">
                        <div class="col-md-4">
                            <?=
                                    ...
                                    ->textInput(['maxlength' => true, 'placeholder' => Yii::t('app', 'Work Title')])->label(false)
                            ?>
                        </div>
                        <div class="col-md-4">
                            <?=
                                    $form->field($model, 'url', ['template' => '{label}<div class="">{input}{error}</div>'])
                                            ...
                                            <?php foreach ($imagePaths as $key => $imagePath): ?>
                                                <div style="display: inline-block;"
                                                    ...
                                                    <span class="close remove">
                                                        <i class="fa fa-close icon-close-works"></i>
                                                    </span>
                                                    <div class="panel p6 pbn">
                                                        <div class="of-h">
                                                            <?php
                                                            echo Html::img('/uploads/images/work/'.$model->id.'/' . $imagePath['path'], [
                                                                'class' => 'img-responsive',
                                                                'title' => $model->name,
                                                                'alt' => '',
                                                            ])
                                                            ?>
                                                            <div class="row table-layout change_image"data-key="<?php echo $imagePath['id'] ?>" product-id="<?=$model->id?>">
                                                                <input type="hidden" value="works" id="category">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <?php
                        if (!$model->isNewRecord) {
                            echo Html::a(Yii::t('app', 'Reset'), Url::to('/' . Yii::$app->language . '/works/index', true), ['class' => 'btn btn-default btn-sm ph25 reste-button pull-right']);
                        }
                        ?>
                    </div>
                    <?php ActiveForm::end(); ?>
...
<?php echo $this->registerJs("
            CKEDITOR.replace('works-short_description');
            CKEDITOR.replace('works-description');
"); ?>
<?php
$this->registerJs("
$('#works-name').on('focusout',function(){
