...
class Service extends \yii\db\ActiveRecord implements CartPositionInterface
{
    ...
    public static function find_One($url = ''){
        ...
        $query = (new Query());
        $query->select(['tr_service.*','service.route_name']);
        $query->from('service');
        if($url != ''){
            $where = array_merge($where,['service.route_name' => $url]);
        }
        $query->leftJoin('tr_service', 'service.id = tr_service.service_id');
        $query->leftJoin('language', 'language.id = tr_service.language_id');
        $query->where($where);
       // $query->offset(1);
        $query->orderBy(['service.id' => SORT_DESC]);
        $query->limit(1);
        $rows = $query->all();
