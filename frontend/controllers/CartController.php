...
class CartController extends Controller {
    ...
    public function actionList($id = NULL) {
        ...
        if(!Yii::$app->user->isGuest && !empty(Yii::$app->user->identity->customer->customerAddresses)){
            $country = Countries::findOne(['status'=>1,'name'=>Yii::$app->user->identity->customer->customerAddresses[0]->country]);
           if($country){
            ...
            $countryID = $arrCountries[0]['id'];
        }

        $arrFreightMethods = Zones::findZones($countryID,$session->get('basketTotalWeight'));
        ...
        return $this->render('order', [
            ...
            'basketProducts' => $session->get('basket')['packages'],
        ...
        if (\Yii::$app->request->isAjax) {
            ...
            if($customer->save(false)){
                if($coordinate = $this->ValidateAddress($shippingInfo)){
                    $customerAddress->city = $shippingInfo['city'];
                    $customerAddress->country = $shippingInfo['country'];
